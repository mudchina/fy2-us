...
int process_list(string idx, mapping info)
{
	...
	while (j--) if (sscanf(inf[j], "%s:%s", name, value) == 2)
		newok[name] = value;
	if(!newok["NAME"]) return 0;

	// make sure the name is in the proper form
	name = htonn( newok["NAME"] );
	while( name[strlen(name)-1] == '.' )
		name = name[ 0..strlen(name)-2 ];
	newok["ALIAS"] = nntoh(newok["NAME"]);

	// already know about ourselves
	if (newok["NAME"] == Mud_name()) return 0;
	...
	if (!old) DNS_MASTER->set_mud_info(name, newok);
	else {
		inf = keys(newok);
		j = sizeof(inf);
		while (j--) old[inf[j]] = newok[inf[j]];
