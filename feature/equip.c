...
int wear()
{
	...
	for(i = 0; i<sizeof(apply); i++)
		if( undefinedp(applied_prop[apply[i]]) )
			...
			applied_prop[apply[i]] += armor_prop[apply[i]];
        if( mapp(armor_prop = query("unique_armor_prop")) )
	{
        apply = keys(armor_prop);
        applied_prop = owner->query_temp("apply");
        if( !mapp(applied_prop) ) applied_prop = ([]);
...
int wield()
{
	...
	for( i = 0; i<sizeof(apply); i++)
        ...
        if( mapp(weapon_prop = query("unique_weapon_prop")) )
	{
	apply = keys(weapon_prop);
	for( i = 0; i<sizeof(apply); i++)
                owner->add_temp("apply/" + apply[i], weapon_prop[apply[i]]);
...
{
	object owner;
	mapping prop, applied_prop, unique_prop;
	...
	if(unique_prop){
        apply = keys(unique_prop);
        applied_prop = owner->query_temp("apply");
        for( i = 0; i<sizeof(apply); i++)
                // To support array apply, we cannot add_temp() here.
                applied_prop[apply[i]] -= unique_prop[apply[i]];
	}
	delete("equipped");
	return 1;
}
