...
//	if( userp(victim) && (int) victim->query("combat_exp") < 2000)
//	return notify_fail("你不能偷新手身上的东西。\n");
...
"身上看起来没有什麽值钱的东西好偷。\n");
	...
	sp = (int)me->query_skill("stealing");
	if( sp > 30)
   log_file( "STEAL_LOG", sprintf("(%s)
%s has stealing skill at level %d\n", ctime(time()), me->query("id"), sp));
	else return notify_fail("凭你现在的能力，还偷不到任何东西。\n"); 
	if( (int) me->query("combat_exp") < 4000 )
	return notify_fail("凭你现在的能力，还偷不到任何东西。\n");
        sp = (int)me->query_skill("stealing") * 5 + (int)me->query_kar()* 2
                - (int)me->query("thief") * 20;
//		modified by Tie 
//        sp = (int)me->query_skill("stealing") * 5 + (int)me->query_kar()* 3;
        if( sp < 1 ) sp = 1;
        if( me->is_fighting() ) {
                sp /= 2;
                me->start_busy(3);
